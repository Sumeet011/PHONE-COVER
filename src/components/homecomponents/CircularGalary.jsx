...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Remove the floating effect by commenting out the wave motion
  ...
  update(scroll, direction) {
    ...
    // Always hide text - only show images
    this.title.hide();
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.baseScaleY = (this.viewport.height * (750 * this.scale)) / this.screen.height;
    ...
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = 4; // Increased padding for better spacing around center card
...
class App {
  ...
  createCamera() {
    ...
    this.camera.fov = 45;
    this.camera.position.z = 25; // Increased from 20 to 25
  }
  createScene() {
  ...
  if (window && window.location && window.location.pathname.startsWith('/Specific_Collection')) {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const defaultItems = items || [
      {
        image: "/images/home/cards/card1.jpg",
        text: "Adventure Quest",
        level: "Level 1",
      },
      {
        image: "/images/home/cards/card2.jpg",
        text: "Mystery Manor",
        level: "Level 2",
      },
      {
        image: "/images/home/cards/card3.jpg",
        text: "Space Odyssey",
        level: "Level 3",
      },
      {
        image: "/images/home/cards/card4.jpg",
        text: "Fantasy Realm",
        level: "Level 4",
      },
    ] ;
  ...
  onTouchMove(e) {
    ...
    const dx = point.clientX - this.start;
    const dy = point.clientY - this.startY;

    if (isTouchEvent) {
      if (!this.gestureAxis) {
        if (Math.abs(dx) < 6 && Math.abs(dy) < 6) return;
        this.gestureAxis = Math.abs(dx) > Math.abs(dy) ? "x" : "y";
      }
    ...
    const distance = (this.start - point.clientX) * (this.scrollSpeed * 0.1);
    this.scroll.target = this.scroll.position + distance;
  }

  onTouchUp(e) {
    const isTouchEvent = Boolean(e && (e.changedTouches || e.touches));

    if (!isTouchEvent || this.gestureAxis === "x") {
      this.onCheck();
    }
 ...
 onWheel(e) {
  ...
  onResize() {
    ...
    const fov = (this.camera.fov * Math.PI) / 180; // Changed from 190 to 180
    const height = 2.2 * Math.tan(fov / 2) * this.camera.position.z; // Changed from 1.9 to 2.2
...
export default function CircularGallery({
  ...
  return (
    <div
      className="w-[100%] h-full overflow-hidden cursor-grab active:cursor-grabbing"
