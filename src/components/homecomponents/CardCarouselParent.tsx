...
const CardCarouselParent = () => {
  ...
  useEffect(() => {
    // Fetch hero carousel images from backend with legacy fallback.
    const fetchImages = async () => {
      try {
        ...
        const fetchByCategory = async (category: string) => {
          const response = await fetch(`${BACKEND_URL}/api/design-assets?category=${category}&isActive=true`);
          return response.json();
        };

        let data = await fetchByCategory('HERO');

        if (!(data.success && data.items && data.items.length > 0)) {
          data = await fetchByCategory('CARD');
        }
  ...
  return (
    <div className="relative">
      ...
      <div onClick={handleOnClick} className="cursor-pointer relative bottom-13 flex items-center justify-center left-1/2 transform -translate-x-1/2 z-20">
            <button className="bg-[#9AE600] text-black cursor-pointer  text-2xl sm:text-4xl font-bold py-2 px-4 rounded-full hover:bg-green-600 transition duration-300"> 
              BUY NOW
            </button>
