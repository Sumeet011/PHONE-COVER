...
import SwipeCard from '@/components/homecomponents/SwipeCard';
...
const ProfilePage = () => {
      ...
      try {
        ...
        const gamingCollectionsForSwipe = (profile.gamingCollections || []).map(collection => {
          ...
          const images = collection.cards?.map(card => {
            console.log('Card:', card.name, 'Images:', card.images || card.image, 'Level:', card.level)
            return {
              src: (card.images && card.images.length > 0 ? card.images[0] : card.image) || '/images/card.webp',
              alt: card.name,
              level: card.level
            }
          }) || []
        ...
        const formattedStandardCards = (profile.standardProducts || []).map(product => {
          console.log('Standard product:', product.name, 'Image:', product.images || product.image)
          return {
            src: (product.images && product.images.length > 0 ? product.images[0] : product.image) || '/images/card.webp',
            alt: product.name
          }
        })
  ...
  return (
    <div className="min-h-screen bg-[#090701] overflow-x-hidden">
      ...
      <div className="w-full  max-w-7xl mx-auto pt-30 pb-32 px-4 sm:px-6 lg:px-8">
        {/* Page Title */}
