...
const CheckoutPage = () => {
    ...
    const userId = userdata ? JSON.parse(userdata).id : null;
      ...
      if (result.success && result.data && result.data.items) {
        ...
        const transformedItems = result.data.items.map((item: any) => {
          // Match mycart page logic for detecting gaming collections
          ...
          // Get image from multiple sources like mycart page
          ...
          return {
            ...
            name: item.productDetails?.name || item.collectionName || item.productName || 'Product',
            ...
            // Include additional fields for complete order data
            productOption: item.productOption || 'none',
            collectionType: item.collectionType || 'none',
            productRef: item.productRef || (item.type === 'collection' ? 'Collection' : item.type === 'suggested' ? 'SuggestedProduct' : 'Product'),
            itemRef: item.productRef || (item.type === 'collection' ? 'Collection' : item.type === 'suggested' ? 'SuggestedProduct' : 'Product'),
  ...
  return (
    <div className="bg-[#090701] min-h-screen overflow-x-hidden">
      <Navbar />
      <div className="min-h-screen text-white px-4 md:px-12 py-8 flex flex-col lg:flex-row gap-8 mt-25">
        {/* Left: Delivery Form */}
        <PersonDetails 
          totalCost={totalCost}
